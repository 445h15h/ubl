...
Uint32 NANDBOOT_copy()
{
  ...
  Uint32 count,blockNum,startBlock,endBlock;
  Uint32 i;
  Uint32 magicNum;
...
#ifndef IPNC_DM365
	/* Check  is Diagnostic  mode ?*/
	for (i = 0l; i < PRESS_TIME; i++) {
		if (GPIO->INDTATA45 & (1<<18))	/*check alm_in key press*/
			break;
	}
	if (i == PRESS_TIME) {
		startBlock = DEVICE_NAND_DBL_SEARCH_START_BLOCK;
		endBlock = DEVICE_NAND_DBL_SEARCH_END_BLOCK;
	} else {
		startBlock = DEVICE_NAND_UBL_SEARCH_START_BLOCK;
		endBlock = DEVICE_NAND_UBL_SEARCH_END_BLOCK;
	}
#else
  startBlock = DEVICE_NAND_UBL_SEARCH_START_BLOCK;
  endBlock = DEVICE_NAND_UBL_SEARCH_END_BLOCK;
